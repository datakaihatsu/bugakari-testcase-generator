...
class ColumnTCGenerator:
    ...
    def generate(self):
        ...
        self._axes_columns_out = list(axes_columns)   # 第2弾G条件が列->質問No対応を得る用(出力不変)
        ...
        for tc_idx, (combo, tcw_data) in enumerate(zip(combos, tc_walks), 1):
            ...
            for (ax, _), chosen_row in zip(vary_row_lists, combo):
                ...
                if self.old_json is None:
                    ...
                    checks.append(check_text)
                    # 案C (2026-09-02 ユーザ合意): 文字だけでは「同じ物の名前直し」と「別物への
                    #   置き換え」を判別できない(例 12 重建設機械分解組立: 表記統一14件の中に
                    #   しきい値 1.4m3→1.5m3 の実体変更が混在)。文字修正行でもテスターが計算結果
                    #   まで確認するよう、この行に固有の観点として明示する。
                    #   積算基準改定だけでなく要望対応もあるため「積算基準および設計書」で揃える。
                    checks.append('・文字修正後の選択肢を選んだとき、代価行・数量・単価が'
                                  '積算基準および設計書どおりであること'
                                  '（文字だけの修正か、内容の変更かを確認）')
                elif '新規到達' in reason:
