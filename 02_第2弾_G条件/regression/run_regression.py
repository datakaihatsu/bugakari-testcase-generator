...
使い方:
    python3 02_第2弾_G条件/regression/run_regression.py                  # 全工種 実行+比較
    python3 02_第2弾_G条件/regression/run_regression.py --only 02,06     # id 前方一致で絞り込み
    python3 02_第2弾_G条件/regression/run_regression.py --no-run         # 再実行せず比較のみ
    python3 02_第2弾_G条件/regression/run_regression.py --update-baseline 02_605773_大型ブレーカ
    python3 02_第2弾_G条件/regression/run_regression.py --update-baseline all

終了コード: 0=全一致 / 1=差分あり / 2=実行エラーあり

運用ルール:
  - スクリプト(02_第2弾_G条件/engine, step1_5, step2_proposals, step3_csv, pipeline.py)を
    改修したら、コミット前に必ず実行する。
...
def run_target(t, log):
    ...
    try:
        if t['mode'] == 'diff':
            cmds = [[sys.executable, str(ROOT / '02_第2弾_G条件/pipeline.py'),
                     str(d / 'input' / t['old']), str(d / 'input' / t['new']), str(out)]]
        else:  # new
            step3_cmd = [sys.executable, str(ROOT / '02_第2弾_G条件/step3_csv/generate_csv.py'),
            ...
            cmds = [
                [sys.executable, str(ROOT / '02_第2弾_G条件/step2_proposals/generate_proposals_new.py'),
