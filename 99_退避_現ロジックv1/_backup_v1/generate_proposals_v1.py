...
class TestPlanGenerator:
    ...
    def _is_ui_visible_axis(self, sit):
        ...
        2. ShortCutSitsumonNo を持つ → 別 Sit への参照コピー (UI 不可視)
        3. Mesho が "Var=値" 形式の内部ラベル (`A=10`, `NFG1=2`, `0=代価表1枚目4行目` 等)
        4. SitTabRows が小さく(<=4) AutoSelectJoken付き行が1つだけ
           → 内部の自動分岐確定パターン
        """
        kind = sit.get('SitsumonKind')
        if kind not in (17, 19):
            return False
        # ShortCutSitsumonNo: 別の Sit へのショートカット参照 → UI 不可視 (重複)
        ...
        if sit019 is not None:
            ...
            # AutoSelectJoken に MinKigou (下限指定) を含む Sit は内部分岐質問:
            #   - ユーザ選択質問: MaxKigou=3 (<=) のみで連続選択肢を構築
            #   - 内部分岐質問:   MinKigou を含む範囲指定 or 等号指定で自動分岐
            # 例: Sit 7 (賃料損料区分) は 1.0<Nfg2<10.0 / 11.0<Nfg2<20.0 → 内部
            #     Sit 2 (積算方法の選択) は FG2<=1.0 / FG2<=3.0 / FG2<=4.0 → ユーザ選択
            has_min_kigou = False
            joken_count = 0
            for r in rows:
