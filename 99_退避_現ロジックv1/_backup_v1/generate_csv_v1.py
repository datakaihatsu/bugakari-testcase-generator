...
class ColumnTCGenerator:
    ...
    def generate(self):
        # C: 列順を フロー登場順 にする (baseline_walker での初回出現順)
        #   baseline_walker は後段でも実行するが、ここでは並び順決定のために先行実行
        ...
        for idx, sn in enumerate(_visit_seq):
            if sn not in _visit_order:
                _visit_order[sn] = idx
        # フロー上で訪問されなかった axis は末尾 (一意な大きな値) に
        ...
        # === 全 TC walker を事前実行: visited セットと daika_row_flags を確定 ===
        #   walker は内部状態を持つので combo ごとに新規インスタンス化
        tc_walks = []  # [{'visited': set, 'daika_flags': dict}]
        all_visited = set(baseline_result.get('visited_sitsumons', []))
        for combo in combos:
            tc_vary_sels = {}
        ...
        # === A: 到達しない質問を列から除外 ===
        #   vary 軸は安全策として常に残す (vary値で訪問されないケースの保険)
        ...
        if axes_excluded:
            print(f'  [A] 到達せず除外: {len(axes_excluded)}件 -> '
