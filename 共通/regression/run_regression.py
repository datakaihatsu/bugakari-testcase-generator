...
使い方:
    python3 共通/regression/run_regression.py                  # 全工種 実行+比較
    python3 共通/regression/run_regression.py --only 02,06     # id 前方一致で絞り込み
    python3 共通/regression/run_regression.py --no-run         # 再実行せず比較のみ
    python3 共通/regression/run_regression.py --update-baseline 02_605773_大型ブレーカ
    python3 共通/regression/run_regression.py --update-baseline all

終了コード: 0=全一致 / 1=差分あり / 2=実行エラーあり

運用ルール:
  - スクリプト(共通/engine, step1_5, step2_proposals, step3_csv, pipeline.py)を
    改修したら、コミット前に必ず実行する。
