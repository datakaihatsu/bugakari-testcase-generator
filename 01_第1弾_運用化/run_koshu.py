...
【使い方】
  python3 01_第1弾_運用化/run_koshu.py                # 工種別/ 配下すべて
  python3 01_第1弾_運用化/run_koshu.py 16 18          # 先頭一致で工種を指定
"""
...
def run_folder(folder):
    ...
    if len(js) >= 2:
        old, new = js[0], js[-1]
        cmds = [[sys.executable, os.path.join(ROOT, '01_第1弾_運用化/pipeline.py'), old, new, out_dir,
                 '--external-scenarios']]
        mode = f'差分型 (旧={os.path.basename(old)} / 新={os.path.basename(new)})'
    else:
        new = js[0]
        plan = os.path.join(out_dir, 'step2.0_テスト計画.csv')
        step3_cmd = [sys.executable, os.path.join(ROOT, '01_第1弾_運用化/step3_csv/generate_csv.py'),
        ...
        cmds = [
            [sys.executable, os.path.join(ROOT, '01_第1弾_運用化/step2_proposals/generate_proposals_new.py'), new, plan],
            step3_cmd,
        ]
    for cmd in cmds:
